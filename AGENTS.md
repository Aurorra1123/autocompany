...
- 不要把关键背景只留在聊天中
- agent 可以自由记录原始实验、日志、指标、观察和临时解释到 `harness/verification/experiments/`
- 写入或修改 durable project truth 前必须停下来向用户确认，包括 accepted/reviewed finding、comparison 结论、idea 索引、路线图、项目 brief、ADR 和会影响后续 agent 判断的结论
- 请求确认时必须给出拟写入的结论或 idea、证据路径和目标文件；得到明确批准后才能落盘
- 不要把实验观察静默提升为团队结论或项目记忆
